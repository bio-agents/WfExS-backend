...
class WfExSBackend:
    ...
    def __init__(
        ...
        agentSect = local_config.get("agents", {})
        # Populating paths
        for keyC, pathC in agentSect.items():
        ...
        encfsSect = agentSect.get("encrypted_fs", {})
        ...
        # The agentbox to be shared with others
        self.rocrate_agentbox = ROCrateAgentbox(self)
    ...
    def cacheWorkflow(
        ...
        if parsedRepoURL.scheme in ("", TRS_SCHEME_PREFIX):
            ...
            if parsedRepoURL.scheme == TRS_SCHEME_PREFIX:
                ...
                trs_steps.extend(["ga4gh", "trs", "v2", "agents"])
    ...
    def getWorkflowRepoFromTRS(
        ...
        # Now, check the agent does exist in the TRS, and the version
        trs_agents_url = cast(
            "URIType",
        ...
        trs_cached_agent = self.cacheHandler.fetch(
            trs_agents_url, destdir=meta_dir, offline=offline, ignoreCache=ignoreCache
        )
        # Giving a friendly name
        if not trsQueryCache.exists():
            os.symlink(trs_cached_agent.path.name, trsQueryCache)

        with trsQueryCache.open(mode="r", encoding="utf-8") as tQ:
            rawAgentDesc = tQ.read()

        # If the agent does not exist, an exception will be thrown before
        jd = json.JSONDecoder()
        agentDesc = jd.decode(rawAgentDesc)

        # If the agent is not a workflow, complain
        if agentDesc.get("agentclass", {}).get("name", "") != "Workflow":
            raise WFException(
                "Agent {} from {} is not labelled as a workflow. Raw answer:\n{}".format(
                    workflow_id_str, trs_endpoint, rawAgentDesc
                )
            )

        possibleAgentVersions = agentDesc.get("versions", [])
        if len(possibleAgentVersions) == 0:
            raise WFException(
                "Version {} not found in workflow {} from {} . Raw answer:\n{}".format(
                    version_id, workflow_id_str, trs_endpoint, rawAgentDesc
                )
            )

        agentVersion = None
        agentVersionId = str(version_id) if isinstance(version_id, int) else version_id
        if (agentVersionId is not None) and len(agentVersionId) > 0:
            for possibleAgentVersion in possibleAgentVersions:
                if isinstance(possibleAgentVersion, dict):
                    possibleId = str(possibleAgentVersion.get("id", ""))
                    possibleName = str(possibleAgentVersion.get("name", ""))
                    if version_id in (possibleId, possibleName):
                        agentVersion = possibleAgentVersion
                        break
            else:
                raise WFException(
                    "Version {} not found in workflow {} from {} . Raw answer:\n{}".format(
            ...
            agentVersionId = ""
            for possibleAgentVersion in possibleAgentVersions:
                possibleAgentVersionId = str(possibleAgentVersion.get("id", ""))
                if (
                    len(possibleAgentVersionId) > 0
                    and agentVersionId < possibleAgentVersionId
                ):
                    agentVersion = possibleAgentVersion
                    agentVersionId = possibleAgentVersionId
        ...
        agentDescriptorTypes = agentVersion.get("descriptor_type", [])
        if not isinstance(agentDescriptorTypes, list):
            raise WFException(
        ...
        if chosenDescriptorType is None:
            for candidateDescriptorType in self.RECOGNIZED_TRS_DESCRIPTORS.keys():
                if candidateDescriptorType in agentDescriptorTypes:
        ...
        elif chosenDescriptorType not in agentVersion["descriptor_type"]:
            raise WFException(
        ...
        agentFilesURL = (
            trs_agents_url
            + "/versions/"
            + urllib.parse.quote(agentVersionId, safe="")
            + "/"
        ...
        if trs_endpoint_meta.get("organization", {}).get("name") == "WorkflowHub":
            ...
            roCrateURL = cast(
                "URIType",
                agentFilesURL + "?" + urllib.parse.urlencode({"format": "zip"}),
            )
            ...
            cached_trs_files = self.cacheFetch(
                cast("URIType", INTERNAL_TRS_SCHEME_PREFIX + ":" + agentFilesURL),
                CacheType.TRS,
                offline=offline,
                ignoreCache=ignoreCache,
            )
                ...
                if repo is not None:
                    self.logger.debug(
                        "Derived repository {} ({} , rel {}) from {}".format(
                            repo.repo_url, repo.tag, repo.rel_path, trs_agents_url
                        )
                    )
                    return (
                        IdentifiedWorkflow(
            ...
            if workflow_entrypoint is not None:
                self.logger.debug(
                    "Using raw files from TRS agent {}".format(trs_agents_url)
                )
                return (
                    IdentifiedWorkflow(
                        workflow_type=expectedEngineDesc,
                        remote_repo=RemoteRepo(
                            repo_url=cast("RepoURL", agentFilesURL),
        ...
        raise WFException("Unable to find a workflow in {}".format(trs_agents_url))
    ...
    def getWorkflowRepoFromROCrateFile(
        ...
        matched_crate, g = self.rocrate_agentbox.identifyROCrate(
        ...
        repo, workflow_type, _ = self.rocrate_agentbox.extractWorkflowMetadata(
            g,
