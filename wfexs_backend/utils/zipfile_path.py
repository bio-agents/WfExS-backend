...
def _parents(path: "str") -> "Iterator[str]":
    ...
    []
    """
    return iteragents.islice(_ancestry(path), 1, None)
...
def _difference(
    ...
    with O(1) lookup.
    """
    return iteragents.filterfalse(set(subtrahend).__contains__, minuend)
...
class CompleteDirs(zipfile.ZipFile):
    ...
    def _implied_dirs(names: "Sequence[str]") -> "Mapping[str, Optional[str]]":
        parents = iteragents.chain.from_iterable(map(_parents, names))
