...
def fetchTRSFiles(
    ...
    """
    Method to download contents from TRS files related to a agent
    ...
    if parsedInputURL.scheme == INTERNAL_TRS_SCHEME_PREFIX:
        ...
        version_steps.extend(
            ["ga4gh", "trs", "v2", "agents", path_steps[-2], "versions", path_steps[-1]]
        )
...
SCHEME_HANDLERS: "Mapping[str, DocumentedProtocolFetcher]" = {
    ...
    TRS_SCHEME_PREFIX: DocumentedProtocolFetcher(
        fetcher=fetchTRSFiles,
        description="GA4GH TRS metadata is fetched using the APIs described at https://ga4gh.github.io/agent-registry-service-schemas/. Contents are downloaded delegating their associated URIs to other fetchers",
    ),
}
