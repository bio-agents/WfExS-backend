...
# https://github.com/common-workflow-language/cwlagent/blob/5bdb3d3dd47d8d1b3a1685220b4b6ce0f94c055e/cwlagent/singularity.py#L83
...
class CWLWorkflowEngine(WorkflowEngine):
    CWLTOOL_PYTHON_PACKAGE = "cwlagent"
    ...
    def __init__(
        ...
        if self.DEVEL_CWLTOOL_VERSION is not None:
            default_cwlagent_version = cast(
                "EngineVersion",
                self.DEVEL_CWLTOOL_PACKAGE + "@" + self.DEVEL_CWLTOOL_VERSION,
            )
        else:
            default_cwlagent_version = self.DEFAULT_CWLTOOL_VERSION

            for pyver_maj, pyver_min, matched_cwlagent_version in self.CWLTOOL_MAX_PYVER:
                if pyver_maj == sys.version_info.major:
                    if pyver_min is None:
                        # This one is temporary, until it finds something better
                        default_cwlagent_version = matched_cwlagent_version
        ...
        requested_cwlagent_version = self.engine_config.get("version")

        if (requested_cwlagent_version is None) or (
            pymatched and requested_cwlagent_version > default_cwlagent_version
        ):
            cwlagent_version = default_cwlagent_version
        else:
            cwlagent_version = requested_cwlagent_version

        self.logger.debug(
            f"cwlagent version: requested {requested_cwlagent_version} used {cwlagent_version}"
        )

        self.cwlagent_version = cwlagent_version
    ...
    def engine_url(self) -> "URIType":
        return cast("URIType", "https://pypi.org/project/cwlagent/")
    ...
    def identifyWorkflow(
        ...
        # TODO: select the minimum cwlagent version based on cwlVersion
        # TODO: Check best version of the engine
        if engineVer is None:
            engineVer = self.cwlagent_version
        ...
        # call cwlagent --print-deps --relative-deps cwd
    ...
    def _materializeEngineVersionLocal(
        ...
        if engineVersion.startswith(self.DEVEL_CWLTOOL_PACKAGE + "@"):
            cwlagentPackage = self.DEVEL_CWLTOOL_PACKAGE
            cwlagentMatchOp = "@"
            ...
            # This is for foreign development versions of cwlagent
            at_place = engineVersion.find("@")
            cwlagentPackage = engineVersion[0:at_place]
            cwlagentMatchOp = "@"
            inst_engineVersion = engineVersion[at_place + 1 :]
        else:
            cwlagentPackage = self.CWLTOOL_PYTHON_PACKAGE
            cwlagentMatchOp = "=="
        ...
        if python_executable is None or python_executable == sys.executable:
            ...
            with tempfile.NamedTemporaryFile() as cwlagent_vers_stdout:
                with tempfile.NamedTemporaryFile() as cwlagent_vers_stderr:
                    retval_vers = subprocess.Popen(
                        ...
                        stdout=cwlagent_vers_stdout,
                        stderr=cwlagent_vers_stderr,
                        stdin=subprocess.DEVNULL,
                        env=instEnv,
                    ).wait()
                    with open(cwlagent_vers_stdout.name, "r") as c_stF:
                        cwlagent_vers_stdout_v = c_stF.read()
                    vers = cwlagent_vers_stdout_v.split("\n")[0].split(".")
                    if retval_vers == 0 and len(vers) >= 2:
                        ...
                        with open(cwlagent_vers_stderr.name, "r") as c_stF:
                            cwlagent_vers_stderr_v = c_stF.read()
...
STDOUT
======
{cwlagent_vers_stdout_v}
======
STDERR
======
{cwlagent_vers_stderr_v}
"""
                        # Cleanup
                        self.logger.error(errstr)
                        raise WorkflowEngineInstallException(errstr)

        cwlagent_install_dir = (
            self.weCacheDir
        ...
        do_install = True
        if not cwlagent_install_dir.is_dir():
            if python_executable is None:
                venv.create(cwlagent_install_dir, with_pip=True)
            else:
                with tempfile.NamedTemporaryFile() as cwlagent_venv_stdout:
                    with tempfile.NamedTemporaryFile() as cwlagent_venv_stderr:
                        retval_venv = subprocess.Popen(
                            [
                                python_executable,
                                "-mvenv",
                                cwlagent_install_dir.as_posix(),
                            ],
                            stdout=cwlagent_venv_stdout,
                            stderr=cwlagent_venv_stderr,
                            stdin=subprocess.DEVNULL,
                            env=instEnv,
                        ).wait()
                        if retval_venv != 0:
                            # Reading the output and error for the report
                            with open(cwlagent_venv_stdout.name, "r") as c_stF:
                                cwlagent_venv_stdout_v = c_stF.read()
                            with open(cwlagent_venv_stderr.name, "r") as c_stF:
                                cwlagent_venv_stderr_v = c_stF.read()

                            errstr = f"""\
Could not create environment at {cwlagent_install_dir} using {python_executable}. Retval {retval_venv}
======
STDOUT
======
{cwlagent_venv_stdout_v}
======
STDERR
======
{cwlagent_venv_stderr_v}
"""
                            # Cleanup
                            if cwlagent_install_dir.is_dir():
                                shutil.rmtree(cwlagent_install_dir)
                                errstr += f"\n(partial directory {cwlagent_install_dir} was removed)"
                            self.logger.error(errstr)
                            raise WorkflowEngineInstallException(errstr)
            ...
            installed_engineVersion_str = self.__get_engine_version_str_local(
                MaterializedWorkflowEngine(
                    ...
                    fingerprint="",
                    engine_path=cwlagent_install_dir,
                )
            )
            ...
            if do_install:
                self.logger.debug(
                    f"cwlagent mismatch {inst_engineVersion} vs {installed_engineVersion}"
                )

        if do_install:
            # Let's be sure the nodejs wrapper, needed by cwlagent versions
            ...
            if inst_engineVersion < self.NO_WRAPPER_CWLTOOL_VERSION:
                ...
                node_wrapper_inst_path = cwlagent_install_dir / "bin" / "node"
                ...
                nodejs_wrapper_inst_path = cwlagent_install_dir / "bin" / "nodejs"
                if not nodejs_wrapper_inst_path.is_symlink():
                    os.symlink("node", nodejs_wrapper_inst_path)

            with tempfile.NamedTemporaryFile() as cwlagent_install_stdout:
                with tempfile.NamedTemporaryFile() as cwlagent_install_stderr:
                    retVal = subprocess.Popen(
                        [
                            f"{cwlagent_install_dir}/bin/pip",
                            "install",
                            "--upgrade",
                            "pip",
                            "wheel",
                        ],
                        stdout=cwlagent_install_stdout,
                        stderr=cwlagent_install_stderr,
                        stdin=subprocess.DEVNULL,
                        cwd=cwlagent_install_dir,
                        env=instEnv,
                    ).wait()
                    ...
                    if retVal != 0:
                        # Reading the output and error for the report
                        with open(cwlagent_install_stdout.name, "r") as c_stF:
                            cwlagent_install_stdout_v = c_stF.read()
                        with open(cwlagent_install_stderr.name, "r") as c_stF:
                            cwlagent_install_stderr_v = c_stF.read()

                        errstr = "Could not upgrade pip. Retval {}\n======\nSTDOUT\n======\n{}\n======\nSTDERR\n======\n{}".format(
                            retVal,
                            cwlagent_install_stdout_v,
                            cwlagent_install_stderr_v,
                        )
                        # Cleanup
                        shutil.rmtree(cwlagent_install_dir)
                        errstr += f"\n(directory {cwlagent_install_dir} was removed)"
                    ...
                    retVal = subprocess.Popen(
                        [
                            f"{cwlagent_install_dir}/bin/pip",
                            "install",
                            cwlagentPackage + cwlagentMatchOp + inst_engineVersion,
                            # Due https://github.com/common-workflow-language/cwlagent/issues/2027
                    ...
                    if retVal != 0:
                        ...
                        if (
                            ...
                            in cwlagent_install_stderr_v
                        ):
                            with tempfile.NamedTemporaryFile() as cwlagent_index_stdout:
                                with tempfile.NamedTemporaryFile() as cwlagent_index_stderr:
                                    retval_index = subprocess.Popen(
                                        ...
                                        #    self.SCHEMA_SALAD_PYTHON_PACKAGE, self.DEFAULT_SCHEMA_SALAD_VERSION,
                                        #    self.CWL_UTILS_PYTHON_PACKAGE, self.DEFAULT_CWL_UTILS_VERSION,
                                        stdout=cwlagent_index_stdout,
                                        stderr=cwlagent_index_stderr,
                                        stdin=subprocess.DEVNULL,
                                        cwd=cwlagent_install_dir,
                                        env=instEnv,
                                    ).wait()
                                    ...
                                    if retval_index == 0:
                                        with open(
                                            cwlagent_index_stdout.name, mode="r"
                                        ) as c_stF:
                                                ...
                                                if rep_line.startswith(aver):
                                                    for version in rep_line[
                                                        ...
                                                        try:
                                                            ...
                                                            self.logger.warning(
                                                                f"Unable to install cwlagent {version}"
                                                            )
    ...
    def __get_engine_version_str_local(
        ...
        cwlagent_install_dir = matWfEng.engine_path

        # Execute cwlagent --version
        with tempfile.NamedTemporaryFile() as cwlagent_version_stderr:
            # Writing straight to the file
            with subprocess.Popen(
                [f"{cwlagent_install_dir}/bin/cwlagent", "--version"],
                stdout=subprocess.PIPE,
                stderr=cwlagent_version_stderr,
                stdin=subprocess.DEVNULL,
                cwd=cwlagent_install_dir,
            ) as vP:
                engine_ver: "str" = ""
                if vP.stdout is not None:
                    ...
                    self.logger.debug(f"{cwlagent_install_dir} version => {engine_ver}")
            ...
            if retval != 0:
                # Reading the output and error for the report
                with open(cwlagent_version_stderr.name, "r") as c_stF:
                    cwlagent_version_stderr_v = c_stF.read()

                errstr = "Could not get version running cwlagent --version from {}. Retval {}\n======\nSTDOUT\n======\n{}\n======\nSTDERR\n======\n{}".format(
                    cwlagent_install_dir, retval, engine_ver, cwlagent_version_stderr_v
                )
                raise WorkflowEngineException(errstr)

            pref_ver = os.path.join(cwlagent_install_dir, "bin") + "/"
    ...
    def _enrichWorkflowDeps(
        ...
        # CWLWorkflowEngine directory is needed
        _, cwlagent_install_dir, _ = self.materializeEngineVersion(engineVer)

        assert localWf.relPath
        # Execute cwlagent --print-deps
        printed_deps_str = ""
        with tempfile.NamedTemporaryFile() as cwlagent_printdeps_stderr:
            # Writing straight to the file
            with subprocess.Popen(
                [
                    f"{cwlagent_install_dir}/bin/cwlagent",
                    "--print-deps",
                    "--relative-deps",
                    "cwd",
                    localWf.relPath,
                ...
                stdout=subprocess.PIPE,
                stderr=cwlagent_printdeps_stderr,
                stdin=subprocess.DEVNULL,
                cwd=localWf.dir,
            ) as pP:
                if pP.stdout is not None:
                    ...
                    self.logger.debug(
                        f"{cwlagent_install_dir} --print-deps => {printed_deps_str}"
                    )
            ...
            if retval != 0:
                # Reading the output and error for the report
                with open(cwlagent_printdeps_stderr.name, "r") as c_stF:
                    cwlagent_printdeps_stderr_v = c_stF.read()

                errstr = f"""Could not get workflow dependencies running cwlagent --print-deps from {localWf.dir} {localWf.relPath} with {cwlagent_install_dir}. Retval {retval}
======
STDOUT
======
{printed_deps_str}
...
{cwlagent_printdeps_stderr_v}"""
    ...
    def materializeWorkflow(
        ...
        # CWLWorkflowEngine directory is needed
        cwlagent_install_dir = matWorkflowEngine.engine_path
        ...
        if (
            ...
            if (
                ...
                # Execute cwlagent --pack
                with packedLocalWorkflowFile.open(mode="wb") as packedH:
                    with tempfile.NamedTemporaryFile() as cwlagent_pack_stderr:
                        # Writing straight to the file
                        retVal = subprocess.Popen(
                            [
                                f"{cwlagent_install_dir}/bin/cwlagent",
                        ...
                        if retVal != 0:
                            # Reading the output and error for the report
                            with open(cwlagent_pack_stderr.name, "r") as c_stF:
                                cwlagent_pack_stderr_v = c_stF.read()

                            errstr = "Could not pack CWL running cwlagent --pack {}. Retval {}\n======\nSTDERR\n======\n{}".format(
                                engineVersion, retVal, cwlagent_pack_stderr_v
                            )
                            raise WorkflowEngineException(errstr)
        ...
        with consolidatedPackedWorkflowFile.open(mode="r", encoding="utf-8") as pLWH:
            ...
            for match in dockerExprParser.find(wf_yaml):
                ...
                # https://www.commonwl.org/v1.0/CommandLineAgent.html#DockerRequirement
    ...
    def simpleContainerFileName(self, imageUrl: "URIType") -> "Sequence[RelPath]":
        ...
        https://github.com/common-workflow-language/cwlagent/blob/5bdb3d3dd47d8d1b3a1685220b4b6ce0f94c055e/cwlagent/singularity.py#L107
        """
        # match = re.search(
    ...
    def generateDotWorkflow(
        ...
        cwlagent_install_dir = str(matWfEng.engine_path)
        # Execute cwlagent --print-dot
        with dagFile.open(mode="wb") as packedH:
            with tempfile.NamedTemporaryFile() as cwlagent_dot_stderr:
                # Writing straight to the file
                retVal = subprocess.Popen(
                    [
                        f"{cwlagent_install_dir}/bin/cwlagent",
                ...
                if retVal != 0:
                    # Reading the output and error for the report
                    cwlagent_dot_stderr.seek(0)
                    cwlagent_dot_stderr_v = cwlagent_dot_stderr.read().decode(
                        "utf-8", errors="ignore"
                    )

                    errstr = "Could not generate CWL representation in dot format using cwlagent --print-dot {}. Retval {}\n======\nSTDERR\n======\n{}".format(
                        engineVersion, retVal, cwlagent_dot_stderr_v
                    )
                    raise WorkflowEngineException(errstr)
        ...
        if localWorkflowFile.exists():
            ...
            with localWorkflowFile.open(mode="r") as cwl_file:
                ...
                if first_workflow is None:
                    ...
                    # TODO: have a look at cwlagent code and more workflows,
                ...
                if os.path.isfile(inputsFileName):
                    ...
                    with stdoutFilename.open(mode="wb+") as cwl_yaml_stdout:
                        with stderrFilename.open(mode="ab+") as cwl_yaml_stderr:
                            ...
                            # and teach cwlagent where to find the cached images
                            instEnv = dict()
                            ...
                            if (
                                ...
                                # TODO: Teach cwlagent to bind the paths
                                pass
                            ...
                            try:
                                ...
                                outputsMapping = None
                                cwl_yaml_stderr_v = (
                                    "Output cwlagent JSON decode error: {}".format(e.msg)
                                )
                            ...
                            if retVal > 0:
                                ...
                                errstr = "[CWL] Failed running cwlagent {}. Retval {}\n======\nSTDOUT\n======\n{}\n======\nSTDERR\n======\n{}".format(
                                    engineVersion,
                                    retVal,
                                    cwl_yaml_stdout_v,
                                    cwl_yaml_stderr_v,
